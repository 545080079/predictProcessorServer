...
func sum(input model.InputMap) *CallResp {
	...
	arr := []float64{
		jsonutils.StrToFloat64(input["k1"]),
		jsonutils.StrToFloat64(input["k2"]),
		jsonutils.StrToFloat64(input["k3"]),
	}
	var res float64 = 0
	startTime := time.Now()
	for _, v := range arr {
	...
	return &CallResp{
		Result:   BuildResult(map[string]string {
			"Result": jsonutils.Float64ToStr(res),
		}),
...
func find(input model.InputMap) *CallResp {
	...
	target := jsonutils.StrToFloat64(input["target"])
	for i, v := range arr {
